...
//	strcpy(nf, filename);
	...
	while (count != 1) {
		...
		if (strcmp(temp,"-i") == 0) strcpy(benchmark, arguments[count - 1]);
			...
			if (strcmp(temp,"-L1") == 0) L1=atoi(arguments[count - 1]);
		else
			if (strcmp(temp,"-L2") == 0) L2=atoi(arguments[count - 1]);
		else
			if (strcmp(temp,"-L3") == 0) L3=atoi(arguments[count - 1]);
		else
...
int main(int argc, char **argv) {
	...
	printf("graph_name=%s seed=%d best_found=%d \n",benchmark, seed, alreadybest);
	srand(seed);
...
/*******************************************************/
	setdataStrucGraph();
	cb_final=memSearch(alreadybest, 2);
	freedataStrucGraph();
