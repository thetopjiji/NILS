...
double judge_cbs(int oldcbs, int newcbs,int *oldwc, int *newwc,int num_wc){
	...
	for(o=num_wc-1;oldwc[o]==0;o--);
	cb_old=o;

	return ((cb_new-cb_old)+(newcbs-oldcbs)*1.0/maxi_cbs);
}
...
double judge_fe(int *oldwc, int *newwc, int num_wc){
	...
	return (cb_new+newwc[cb_new]*1.0/num_edge-cb_old-1.0*oldwc[cb_old]/num_edge);
}
...
//	return (cb_new+newwc[cb_new]*1.0/num_edge-cb_old-1.0*oldwc[cb_old]/num_edge);
	if(cb_new>cb_old) va_re=cb_new-cb_old;
	else if(cb_new<cb_old) va_re=cb_new-cb_old;
	else if(cb_new==cb_old) va_re=newwc[cb_new]-oldwc[cb_old];
	else {
		cout<<"error in judge_part"<<endl;
		exit(-1);
	}
...
//	cb_old=o;
//
//	for(i=cb_new;i>=upp*cb_new;i--)	stack_new=stack_new+newwc[i];
//	for(i=cb_old;i>=upp*cb_old;i--)	stack_old=stack_old+oldwc[i];
//
//	fe_new=cb_new+stack_new*1.0/num_edge;
//	fe_old=cb_old+stack_old*1.0/num_edge;
//
//	return (fe_new-fe_old);
...
void OX_modified(int *Father, int *Mother, int length, int *c1, int *c2, int *Fcbnodes, int *Mcbnodes, int *PermuF, int *PermuM, int cb){
	...
	count=0;
	for(i=0;i<length;i++){
		if(Mcbnodes[i]<0.5*cb) {		// i refers to the number of node
			seq[count]=PermuM[i];		// PermuM[i] refers to the label
			count++;					// Count the number of nodes shift
		}
	}
	//copy Father to c1
	for(i=0;i<length;i++) c1[i]=Father[i];
	for(i=0;i<length;i++) pos2[i]=0;
	//find the corresponding positions in F
	for(i=0;i<count;i++){
		for(j=0;j<length;j++){
			if(Father[j]==Mother[seq[i]]){
				pos2[j]=1;
				break;
			}
		}
	...
	if(count!=anothercount) {
		cout<<"error in modified OX2"<<endl;
		exit(-3);
	}
	//get exchange
	for(i=0;i<count;i++)
		c1[seq2[i]]=Mother[seq[i]];
	...
	for(i=0;i<length;i++) {
		if(temp[c1[i]])	{
			cout<<"examine error in modified OX2"<<endl;
			exit(-3);
		}
		else temp[c1[i]]=1;
	}
...
//	if(pt_return==-1) {
//		cout<<pt_set <<" "<<temp_2[0]<<" "<<temp_2[1]<<endl;
//		cout<<st_pos[pt_set] <<" "<<st_pos[temp_2[0]]<<" "<<st_pos[temp_2[1]]<<endl;
//		for(i=0;i<length;i++) cout<<c1[i]<<" ";
//		cout<<endl;
//	}
